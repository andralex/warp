...
@wp.kernel
def copy(A: wp.array2d(dtype=float),
...
#     wp.launch_tiled(copy, dim=(nx//BLOCKSIZE, ny//BLOCKSIZE), inputs=[A], outputs=[C], device="cuda:0", partition=result.partition, offset=offset, block_dim=32)
#     print(C.numpy())

C = wp.zeros((nx,ny), dtype=float)
wp.launch_tiled(copy, dim=(nx//BLOCKSIZE, ny//BLOCKSIZE), inputs=[A], outputs=[C], device="cuda:0", partition="(3,3):(2,12)", offset=7, block_dim=32)
print(C.numpy())
